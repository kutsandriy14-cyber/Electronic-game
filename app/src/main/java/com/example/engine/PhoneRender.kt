...
object PhoneRender {
    ...
    private val metalShineBrush = Brush.linearGradient(listOf(Color.Transparent, Color(0x44FFFFFF), Color.Transparent))
    ...
    fun drawComponent(drawScope: DrawScope, grid: Array<Array<GridComponent>>, x: Int, y: Int, width: Int, height: Int, component: GridComponent, cellSize: Float) {
        ...
        // Slightly brighter high-contrast color scheme for high-DPI phone screens
        val color = if (component.isOverloaded) {
            Color(0xFFFF3E3E)
        } else {
            if (component.isPowered) Color(0xFFD7C4FF) else Color(0xFF5A5565)
        }
        
        // Thicker stroke sizes on phones for visibility and readability
        val strokeSize = cellSize * 0.12f
            ...
            when (type) {
                ComponentType.WIRE_ANY, ComponentType.SUPERCONDUCTOR, ComponentType.HIGH_VOLTAGE_CABLE, ComponentType.FIBER_OPTIC -> {
                    ...
                    val wireColor = if (component.isPowered) {
                        if (isSuper) Color(0xFF00E5FF) else Color(0xFF00FF33)
                    } else {
                        if (isSuper) Color(0xFF114455) else Color(0xFF5A5565)
                    }
                    val wStroke = if (isSuper) strokeSize * 1.6f else strokeSize * 1.3f
                    ...
                    if (connects != 2 || (up && down && left && right)) {
                        drawCircle(wireColor, radius = wStroke * 0.9f, center = Offset(cx, cy))
                    } else if (connects == 0) {
                        drawCircle(wireColor, radius = wStroke * 1.6f, center = Offset(cx, cy))
                    }
                }
                ...
                ComponentType.DIODE -> {
                    ...
                    // Reusable path to prevent GC spike
                    val p = getReusablePath()
                    p.moveTo(cx - padding * 1.5f, cellSize - padding * 2)
                    p.lineTo(cx + padding * 1.5f, cellSize - padding * 2)
                    p.lineTo(cx, padding * 2)
                    p.close()
                    drawPath(p, color)
                ...
                ComponentType.RESISTOR -> {
                    ...
                    drawRect(Color.Yellow, topLeft = Offset(cx - padding, cy - strokeSize * 0.3f), size = Size(padding * 2, strokeSize * 0.3f))
                }
                ...
                ComponentType.BULB -> {
                    val bulbColor = if (component.isPowered) Color(0xFFD8C7FF) else Color(0xFF332D41)
                    if (component.isPowered) {
                        drawCircle(Color(0xFFD0BCFF).copy(alpha = 0.45f), radius = cellSize * 0.48f, center = Offset(cx, cy))
                    }
                ...
                ComponentType.DISPLAY_PIXEL -> {
                    ...
                    if (component.isPowered) {
                        drawRect(Color(0x33FFFFFF), topLeft = Offset(0f, 0f), size = Size(cellSize, cellSize))
                    }
                ...
                ComponentType.LED -> {
                    ...
                    if (component.isPowered) drawCircle(ledColor.copy(alpha = 0.65f), radius = cellSize * 0.45f, center = Offset(cx, cy))
                ...
                ComponentType.SWITCH_OPEN -> {
                    ...
                    drawLine(if (component.isPowered) Color(0xFFD0BCFF) else Color(0xFF5A5565), start = Offset(cx, padding * 2), end = Offset(cx - padding * 2, cellSize * 0.75f), strokeWidth = strokeSize * 0.9f)
                }
                
                ComponentType.SWITCH_CLOSED -> {
                    drawCircle(color, radius = strokeSize, center = Offset(cx, padding * 2))
                    drawCircle(color, radius = strokeSize, center = Offset(cx, cellSize - padding * 2))
                    drawLine(color, start = Offset(cx, padding * 2), end = Offset(cx, cellSize - padding * 2), strokeWidth = strokeSize * 0.9f)
                }

                ComponentType.PUSH_BUTTON -> {
                    drawCircle(color, radius = strokeSize, center = Offset(cx - padding * 1.5f, cy))
                    drawCircle(color, radius = strokeSize, center = Offset(cx + padding * 1.5f, cy))
                    drawRect(if (component.isPowered) color else Color(0xFF5A5565), topLeft = Offset(cx - padding * 2, cy - padding * 1.5f), size = Size(padding * 4, strokeSize * 1.5f))
                }

                ComponentType.LOGIC_AND -> {
                    val p = getReusablePath()
                    p.moveTo(cx - padding * 1.5f, cy - padding * 1.5f)
                    p.lineTo(cx, cy - padding * 1.5f)
                    p.arcTo(androidx.compose.ui.geometry.Rect(cx - padding * 1.5f, cy - padding * 1.5f, cx + padding * 1.5f, cy + padding * 1.5f), -90f, 180f, false)
                    p.lineTo(cx - padding * 1.5f, cy + padding * 1.5f)
                    p.close()
                    
                    drawPath(p, color, style = Stroke(strokeSize))
                ...
                ComponentType.RELAY -> {
                    ...
                    val p = getReusablePath()
                    p.moveTo(cx - padding, padding * 1.5f + strokeSize)
                    p.quadraticTo(cx, cy, cx - padding, cellSize - padding * 1.5f - strokeSize)
                    drawPath(p, color, style = Stroke(strokeSize * 0.5f))
                ...
                ComponentType.MONITOR_OLED, ComponentType.CRT_MONITOR -> {
                    ...
                    drawCircle(Color(r, g, b, 1f), radius = strokeSize * 0.8f, center = Offset(cx, cy - strokeSize * 2))
                }
            }
        }
    }
